...
Scene::Scene() {
  points.reserve(INITIAL_POINT_CAPACITY);
  previewPoints.reserve(INITIAL_POINT_CAPACITY);

  polyPoints.reserve(256);
}

static void bresenhamImpl(std::vector<glm::vec2> &points,
...
static void bresenham(std::vector<glm::vec2> &points, const glm::ivec2 start,
                      const glm::ivec2 end) {
  const auto d = glm::abs(end - start);
  if (d.y < d.x) {
    if (start.x > end.x)
      bresenhamImpl(points, end, start, true);
    else
      bresenhamImpl(points, start, end, true);
  } else {
    if (start.y > end.y)
      bresenhamImpl(points, end, start, false);
    else
      bresenhamImpl(points, start, end, false);
  }
}
...
static void midPointEllipse(std::vector<glm::vec2> &points,
...
static void midpointCircle(std::vector<glm::vec2> &points,
                           const glm::ivec2 start, const glm::ivec2 end) {
...
void Scene::commitEndPoints() {
  points.insert(points.end(), std::make_move_iterator(previewPoints.begin()),
